...
|> Enum.reduce(
  {0, 0},
  fn id, {twos, threes} ->
    char_array = String.split(id, "", trim: true)

    char_array
    |> Enum.uniq()
    |> Enum.map(&Enum.count(char_array, fn c -> &1 == c end))
    |> Enum.uniq()
    |> (&{twos + Enum.count(&1, fn x -> x == 2 end), threes + Enum.count(&1, fn x -> x == 3 end)}).()
  end
)
|> (&(elem(&1, 0) * elem(&1, 1))).()
|> IO.inspect()
...
|> Enum.reduce_while(
  input_list,
  fn id, [_ | t] ->
    Enum.find_value(t, fn x ->
      String.myers_difference(id, x)
      |> (&(if(&1 |> Keyword.get_values(:del) |> Enum.join() |> String.length() == 1) do
              &1 |> Keyword.get_values(:eq) |> Enum.join()
            else
              nil
            end)).()
    end)
